  ...
  return (
    <main className="relative lg:min-h-full">
      <div className="hidden lg:block h-80 overflow-hidden lg:absolute lg:h-full lg:w-1/2 lg:pr-4 xl:pr-12">
        <Image className="h-full w-full object-cover object-center" src="/checkout-thank-you.jpg" fill alt="thank you for your order" />
      </div>
      <div>
          ...
          <div className="lg:col-start-2">
            ...
            {order._isPaid ? (
              <p className="mt-2 text-base text-muted-foreground">
                Your order was processed and your assets are avaliable to download below. We&apos;ve sent your receipt and order details to{' '}
                {typeof order.user !== 'string' ? <span className="font-medium text-gray-900">{(order.user as User).email}</span> : null}.
              </p>
            ) : (
            ...
            <div className="mt-16 text-sm font-medium">
                    ...
                    <li className="flex space-x-6 py-6" key={product.id}>
                      <div className="relative h-24 w-24">
                        {typeof image !== 'string' && image.url ? (
                          <Image className="flex-none rounded-md bg-gray-100 object-cover object-center" src={image.url} fill alt={`${product.name} image`} />
                        ) : null}
                      </div>
                      <div className="flex-auto flex flex-col justify-between">
                        ...
                        {order._isPaid ? (
                          <a className="text-blue-600 hover:underline underline-offset-2" href={downloadUrl} download={product.name}>
                            Download asset
                          </a>
                        ) : null}
                      </div>
              ...
              <PaymentStatus orderId={order.id} orderEmail={(order.user as User).email} isPaid={order._isPaid} />
